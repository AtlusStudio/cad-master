...
function normalizeCeiling(input = {}) {
  const ceiling = {
    ...
    large_room_ratio: numeric(input.large_room_ratio, "独立房间面积比例", 0, false),
    text_height: numeric(input.text_height, "吊顶标注字高", 0, false),
  }
  if (ceiling.large_room_ratio > 1) throw new Error("独立房间面积比例不能大于 1。")
  return ceiling
}
...
export function presetFingerprint(preset) {
  return createHash("sha256")
    .update(JSON.stringify({ drawing: preset.drawing, materials: preset.materials, ceiling: preset.ceiling }))
    .digest("hex")
}
