...
function getDatabase() {
  ...
  `)
  if (!database.prepare("PRAGMA table_info(presets)").all().some((column) => column.name === "ai")) {
    database.exec(`ALTER TABLE presets ADD COLUMN ai TEXT NOT NULL DEFAULT '{"thinking":false}'`)
  }
  if (database.prepare("SELECT COUNT(*) AS count FROM presets").get().count === 0) {
    const timestamp = new Date().toISOString()
    database.prepare(`
      INSERT INTO presets (
        id, name, description, drawing, materials, ceiling, ai, created_at, updated_at, is_default
      ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, 1)
    `).run(
      DEFAULT_PRESET.id,
      DEFAULT_PRESET.name,
      ...
      JSON.stringify(DEFAULT_PRESET.ai),
      timestamp,
      timestamp,
    )
  }
...
function presetFromRow(row) {
  return {
    ...
    ai: { ...DEFAULT_PRESET.ai, ...JSON.parse(row.ai) },
  }
}
...
function normalizeAi(input = {}) {
  if (typeof input.thinking !== "boolean") throw new Error("模型思考开关无效。")
...
export async function savePreset(input) {
  ...
  db.prepare(`
    INSERT INTO presets (id, name, description, drawing, materials, ceiling, ai, created_at, updated_at)
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
